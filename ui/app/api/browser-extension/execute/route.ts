...
export async function POST(request: NextRequest) {
  try {
    const { clicks, urls, domSnapshots, session, parameters, url, taskInstruction, evalInput } = await request.json();

    // Handle two cases:
    // 1. From browser extension: has clicks array
    // 2. From UI with click-through test type: has url and taskInstruction

    const isFromExtension = clicks && Array.isArray(clicks) && clicks.length > 0;
    const isFromUI = url && !clicks;

    if (!isFromExtension && !isFromUI) {
      return new Response(
        JSON.stringify({ error: 'Either clicks array (from extension) or url (from UI) is required' }),
    ...
    // Use the taskInstruction as-is (already in main() format from browser extension or UI)
    const scriptContent = taskInstruction || `async function main(page, stagehand) {
  console.log("No actions specified");
  return { message: "No actions to execute" };
}`;

    // Customer input for custom script workflow
    const customerInput = {
      scriptContent,
      url: url || (urls && urls[0]) || ''
    };
        ...
        try {
          ...
          const workflowPromises = workflowParametersArray.map(async (workflowParameters, index) => {
            try {
              ...
              // Run the custom script workflow (works for both generated and manual scripts)
              const result = await customScriptWorkflow(stagehand, customerInput, evalInput);
          ...
          console.error('[API] Browser extension workflow error:', error);
    ...
    console.error('[API] Browser extension workflow setup error:', error);
    return new Response(JSON.stringify({ error: 'Failed to start browser extension workflow' }), {
      status: 500,
