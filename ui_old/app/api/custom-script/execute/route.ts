...
export async function POST(request: NextRequest) {
  try {
    const { scriptContent, url, evalInput, parameters } = await request.json();
    ...
    const customerInput = { scriptContent, url };
        ...
        try {
          ...
          const workflowPromises = workflowParametersArray.map(async (workflowParameters, index) => {
            try {
              ...
              // Run the custom script workflow
          ...
          console.error('[API] Custom script workflow error:', error);
    ...
    console.error('[API] Custom script workflow setup error:', error);
    return new Response(JSON.stringify({ error: 'Failed to start custom script workflow' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  }
